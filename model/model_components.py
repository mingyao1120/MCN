...
class QueryEncoder(nn.Module):
    '''
    Encode query text into feature embeddings
    '''
...
class FusionModule(nn.Module):
    ...
    def forward(self, video_features, sentence_feature, v_mask):
        sentence_feature_i = sentence_feature
        sentence_feature = sentence_feature[:,None,:] #[B,1,C]
        sentence_feature = sentence_feature.expand_as(video_features) #[B,L,C]

        # ================ Cosine Similarity ================
        # Compute L2 norm
        video_norm = torch.norm(video_features, p=2, dim=-1, keepdim=True)  # (bs, l, 1)
        sentence_norm = torch.norm(sentence_feature_i, p=2, dim=-1, keepdim=True)  # (bs, 1)
        # Avoid division by zero
        video_norm = video_norm.clamp(min=1e-8)
        sentence_norm = sentence_norm.clamp(min=1e-8)
        # Normalize
        video_normalized = video_features / video_norm  # (bs, l, dim)
        sentence_normalized = sentence_feature_i / sentence_norm  # (bs, dim)
        # Compute dot product (cosine similarity)
        cosine_sim = torch.sum(
            video_normalized * sentence_normalized.unsqueeze(1),
            dim=-1
        )  # (bs, l)
        # ===================================================
        fusion_feature = torch.cat([video_features,sentence_feature],dim=-1)
        v_s_concat_1 = self.fc1(fusion_feature*torch.sigmoid(cosine_sim).unsqueeze(-1)) # B,L,C

        logits2 = self.fc3(v_s_concat_1).squeeze(2)   # B,L
        logits2 = mask_logits(logits2,v_mask)
        scores2 = self.sigmoid(logits2)
        
        v_s_concat_2 = self.fc2(fusion_feature*torch.sigmoid(cosine_sim).unsqueeze(-1)) # B,L,C
