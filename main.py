...
from model.model import MCN, build_optimizer_and_scheduler
...
parser.add_argument('--iou_tax', type=str, default='biou', help='')
parser.add_argument("--initializer_range", type=float, default=0.02, help="initializer_range")

# training/evaluation parameters
parser.add_argument("--alpha", type=float, default=1.0, help="alpha for inner_pose loss(boundary loss)")
parser.add_argument("--mom_loss", type=float, default=1.0, help="moment loss for inner_semantic loss(boundary loss)")
...
visual_features = load_video_features(os.path.join('data', 'features', dataset_name, configs.fv), configs.max_pos_len)
...
model_dir = os.path.join(home_dir, str(configs.seed), str(configs.alpha))
...
if configs.mode.lower() == 'train':
    ...
    # build model
    model = MCN(configs=configs, word_vectors=dataset['word_vector']).to(device) 
    ...
    for epoch in range(configs.epochs):
        ...
        for data in tqdm(train_loader, total=num_train_batches, desc='Epoch %3d / %3d' % (epoch + 1, configs.epochs)):
            ...
            ce_loss, bce_loss, bf_loss, iou_loss, mom_loss = model(word_ids, char_ids, vfeats, video_mask, query_mask, s_labels, e_labels, f_labels, h_labels, is_train=True)
            ...
            # print(bce_loss, bf_loss)
            total_loss = ce_loss + configs.gama* bce_loss + configs.beta* bf_loss + configs.alpha*iou_loss  + configs.mom_loss*mom_loss
            ...
            if global_step % eval_period == 0 or global_step % num_train_batches == 0:
                ...
                if  mi_ans >= best_score:
                    ...
                    # only keep the top-5 model checkpoints
                    filter_checkpoints(model_dir, suffix='t7', max_to_keep=5)
    ...
    model_dir="ckpt/MCN_activitynet_RF_i3d_128"
    print(model_dir)
    if not os.path.exists(model_dir):
    ...
    print(f"Found {len(model_filenames)} model files in {model_dir}")
    ...
    print("Sorted model files:", sorted_tuples)

    for step, filename in sorted_tuples:
        print(f"\n{'='*50}")
        print(f"Evaluating model: {os.path.basename(filename)}")
        print(f"{'='*50}")
        ...
        r1i3_ans, r1i5_ans, r1i7_ans, mi_ans, mi_accs, _ = model_eval(
            model=model, 
            data_loader= test_loader, # val_loader, # test_loader, # ,  # val_loader, 
            device=device, 
        ...
        print("\n" + "\x1b[1;31m" + f"Model: {os.path.basename(filename)}" + "\x1b[0m")
        print("\x1b[1;31m" + "Rank@1, IoU=0.3:\t{:.2f}".format(r1i3_ans) + "\x1b[0m", flush=True)
        print("\x1b[1;31m" + "Rank@1, IoU=0.5:\t{:.2f}".format(r1i5_ans) + "\x1b[0m", flush=True)
        print("\x1b[1;31m" + "Rank@1, IoU=0.7:\t{:.2f}".format(r1i7_ans) + "\x1b[0m", flush=True)
        print("\x1b[1;31m" + "{}:\t{:.2f}".format("mean IoU".ljust(15), mi_ans) + "\x1b[0m", flush=True)
